...
begin
  ...
  select id
  into v_collectors_type_id
  from public.container_types
  where lower(code) = 'collectors_box'
  limit 1;

  if v_collectors_type_id is null and v_ocg_type_id is not null then
    update public.container_types
    set
      name = 'Collectors Box',
      code = 'collectors_box',
      description = 'Collectors box products and keys.',
      is_enabled = true,
      updated_at = now()
    where id = v_ocg_type_id;

    v_collectors_type_id := v_ocg_type_id;
  elsif v_collectors_type_id is null then
    ...
    values (
      ...
      'Collectors Box',
      'collectors_box',
      'Collectors box products and keys.',
      true,
      now(),
      now()
    )
    returning id into v_collectors_type_id;
  else
    update public.container_types
    set
      name = 'Collectors Box',
      ...
      is_enabled = true,
      updated_at = now()
    where id = v_collectors_type_id;
  end if;

  if v_ocg_type_id is not null and v_collectors_type_id is not null and v_ocg_type_id <> v_collectors_type_id then
    update public.containers
    set
      container_type_id = v_collectors_type_id,
      updated_at = now()
    where container_type_id = v_ocg_type_id;
...
set
  code = 'random_collectors_box_key',
  name = 'Random Collectors Box Key',
  description = 'Buy this to receive one random key for any unlocked Collectors Box.',
  exact_item_family = 'random_collectors_box_key',
  target_kind = null,
  target_id = null,
  behavior_code = 'grant_random_container_key',
  store_order = coalesce(store_order, 120),
  is_active = true,
  updated_at = now()
where lower(coalesce(code, '')) = 'random_ocg_box_key'
   or lower(coalesce(exact_item_family, '')) = 'random_ocg_box_key'
   or lower(coalesce(code, '')) = 'random_collectors_box_key'
   or lower(coalesce(exact_item_family, '')) = 'random_collectors_box_key';
...
select
  ...
  99,
  0,
  'Buy this to receive one random key for any unlocked Collectors Box.',
  null,
  null,
  'random_collectors_box_key'
from public.item_categories category
where lower(coalesce(category.code, '')) = 'container_openers'
  and not exists (
    select 1
    from public.item_definitions item
    where lower(coalesce(item.code, '')) = 'random_collectors_box_key'
       or lower(coalesce(item.exact_item_family, '')) = 'random_collectors_box_key'
  );

update public.item_definitions
set
  code = public._slugify_store_code(
    replace(
      regexp_replace(lower(coalesce(code, '')), '^ocg_', 'col_'),
      '_ocg_box_key',
      '_collectors_box_key'
    )
  ),
  name = replace(
    replace(coalesce(name, ''), 'OCG-', 'COL-'),
    'OCG Box Key',
    'Collectors Box Key'
  ),
  exact_item_family = 'collectors_box_key',
  updated_at = now()
where lower(coalesce(exact_item_family, '')) = 'ocg_box_key'
   or lower(coalesce(code, '')) like '%_ocg_box_key'
   or coalesce(name, '') ilike '%OCG Box Key%';
...
begin
  ...
  if v_box_category_code = 'collectors_box' then
    return 'COL';
  end if;
...
begin
  ...
  if v_code = 'collectors_box' then
    return 'collectors_box_key';
  end if;
...
begin
  ...
  if v_family = 'collectors_box_key' then
    return 'Collectors Box Key';
  end if;
...
begin
  ...
  if v_family = 'random_collectors_box_key' then
    return 'collectors_box';
  end if;
...
begin
  ...
  return coalesce((
    ...
    join public.container_types ct
      on ct.id = c.container_type_id
    where lower(ct.code) in ('deck_box', 'promo_box', 'collectors_box')
  ), '[]'::jsonb);
end;
$function$;
...
begin
  ...
  join public.container_types ct
    on ct.id = c.container_type_id
  where c.id = p_container_id
    and lower(ct.code) in ('deck_box', 'promo_box', 'collectors_box')
  limit 1;
...
declare
  ...
  v_collectors_tier_violation_count integer := 0;
begin
  perform public._assert_progression_admin();
  ...
  if v_box_category_code = 'collectors_box' then
    select count(*)
    into v_collectors_tier_violation_count
    ...
    where x.card_id is not null
      ...
      and coalesce(tiers.sort_order, 9999) > 5;

    if v_collectors_tier_violation_count > 0 then
      raise exception 'Collectors Boxes only support the first 5 tiers (Bulk through Elite)';
    end if;
  end if;
...
begin
  ...
  if v_container_type_code not in ('promo_box', 'deck_box', 'collectors_box') then
