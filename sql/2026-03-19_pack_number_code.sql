...
alter table public.containers
  add column if not exists pack_number_code text;

update public.containers
set
  pack_number_code = upper(trim(pack_number_code)),
  updated_at = now()
where pack_number_code is not null
  and pack_number_code <> upper(trim(pack_number_code));
...
alter table public.containers
  ...
  check (
    ...
    or upper(trim(pack_number_code)) ~ '^(\\d{3}|RWD)$'
  );
...
as $function$
declare
  v_normalized text := upper(trim(coalesce(p_value, '')));
begin
  if v_normalized = '' then
    raise exception 'Pack Number is required';
  end if;

  if v_normalized !~ '^(\\d{3}|RWD)$' then
    raise exception 'Pack Number must be exactly 3 digits like 001 or RWD';
  end if;
  ...
  return coalesce((
    select jsonb_agg(
      jsonb_build_object(
        ...
        'pack_set_name', nullif(trim(coalesce(full_container.pack_set_name, '')), ''),
      ...
      order by
        lower(coalesce(nullif(trim(coalesce(full_container.pack_set_name, '')), ''), 'zzzz_unsorted')),
        case
          when upper(coalesce(full_container.pack_number_code, '')) ~ '^\\d{3}$' then 0
          when upper(coalesce(full_container.pack_number_code, '')) = 'RWD' then 1
          else 2
        end,
        case
          when upper(coalesce(full_container.pack_number_code, '')) ~ '^\\d{3}$'
            then upper(full_container.pack_number_code)::integer
          else 9999
        end,
...
begin
  ...
  return jsonb_build_object(
    ...
    'pack_set_name', nullif(trim(coalesce(v_full.pack_set_name, '')), ''),
...
begin
  ...
  if v_full_id is null then
    insert into public.containers (
      ...
      pack_set_name, pack_number_code
    )
    values (
      ...
      v_pack_set_name, v_pack_number_code
    )
